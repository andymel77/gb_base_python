...
class Store:
    ...
    def put(self, equipment: OfficeEquipment, count = 1):
        if equipment.id in self._data_store:
            self._data_store[equipment.id]['count'] += count
        else:
            _tmp = {}
            _tmp['equipment'] = equipment
            _tmp['count'] = count
            _tmp['department'] = 'Main'
            self._data_store[equipment.id] = _tmp
        return self._data_store
    
    def move_to_department(self, equipment: OfficeEquipment, new_dep: str):
        try:
            if equipment.id in self._data_store:
                self._data_store[equipment.id]['department'] = new_dep
            else:
